                ...
                tr_variant     * clientSettings)
{
  ...
  /* start the libleechmission thread */
  tr_netInit (); /* must go before tr_eventInit */
  tr_eventInit (session);
  assert (session->events != NULL);

  /* run the rest in the libleechmission thread */
  data.done = false;
...
tr_sessionClose (tr_session * session)
{
  ...
  dbgmsg ("shutting down leechmission session %p... now is %zu, deadline is %zu", session, (size_t)time (NULL), (size_t)deadline);

  /* close the session */
  tr_runInEventThread (session, sessionCloseImpl, session);
  while (!session->isClosed && !deadlineReached (deadline))
    {
      dbgmsg ("waiting for the libleechmission thread to finish");
      tr_wait_msec (100);
    }

  /* "shared" and "tracker" have live sockets,
   * so we need to keep the leechmission thread alive
  ...
  /* close the libleechmission thread */
  tr_eventClose (session);
  while (session->events != NULL)
    {
      static bool forced = false;
      dbgmsg ("waiting for libleechmission thread to finish... now %zu deadline %zu", (size_t)time (NULL), (size_t)deadline);
      tr_wait_msec (100);
